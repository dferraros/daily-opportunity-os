...
> **Purpose:** This section defines how [external] measures the impact of every trigger, detects channel damage before it becomes permanent, and calculates the net business value of each notification. Without measurement, the trigger system cannot prove ROI or justify continued investment. These metrics, dashboards, and test designs give Alvaro and Katy exact specifications to implement from Day 1.
>
> **Owners:**
...
| Avg fatigue_risk score (active users) | < 0.4 | 0.4-0.6 | > 0.6 = reduce P3-P5 frequency | Reduce P3-P5 sends by 50% for 2 weeks; monitor fatigue trend | BigQuery: `SELECT AVG(notification_fatigue_score) FROM [external]_lifecycle.user_profiles WHERE lifecycle_stage NOT IN ('EXCLUDED', 'CHURNED')` |
...
Constants:
  EUR 2.50 = estimated annual push revenue per user (fintech benchmark; calibrate after 30 days with [external] data)
  EUR 0.10 = complaint handling cost per spam report (industry standard)
```

**Why EUR 2.50?** This represents the estimated annual revenue contribution of the push channel per user. When a user opts out of push, [external] permanently loses the ability to drive that user to sessions/trades via push. At avg EUR 12/user/month revenue and push contributing ~1.7% of sessions-to-trade conversion, EUR 2.50/year is a conservative lower bound. Calibrate with actual data after 30 days of MVP.
...
|-----------|-------|--------|
| Eligible users | 72,400 | [external] data: dormant users with balance holding EUR 19.5M AUC |
...
| Avg trade revenue | EUR 12 | [external] avg revenue per trading user per month |
...
-- Output: [external]_lifecycle.vw_nnv_weekly
...
| Property | Value |
|----------|-------|
| **Name** | `[external]_lifecycle.vw_trigger_send_metrics` |
...
| Property | Value |
|----------|-------|
| **Name** | `[external]_lifecycle.vw_trigger_engagement_metrics` |
...
| Property | Value |
|----------|-------|
| **Name** | `[external]_lifecycle.vw_deliverability_health` |
...
| Property | Value |
|----------|-------|
| **Name** | `[external]_lifecycle.vw_nnv_weekly` |
...
| Property | Value |
|----------|-------|
| **Name** | `[external]_lifecycle.vw_holdout_comparison` |
