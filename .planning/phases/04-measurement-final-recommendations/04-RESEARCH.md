...
**Confidence:** MEDIUM -- the formula structure is standard (industry sources MoEngage, Pushwoosh, VWO all converge on revenue-minus-cost framing). The EUR 2.50 annual push revenue estimate needs calibration with Bit2Me data after MVP launch.
...
**Confidence:** MEDIUM -- the gap numbers are from Bit2Me actuals (CLAUDE.md). The lift estimates are directional, based on industry benchmarks, not Bit2Me historical data.
...
### Deliverability Health Thresholds (Recommended for Bit2Me)
...
1. **Bit2Me-specific push opt-in rate unknown**
   - What we know: Industry benchmark for finance apps is 72.3%. Bit2Me's actual rate is not documented.
   - What's unclear: Whether Bit2Me's rate is above or below benchmark. iOS vs Android split for Bit2Me users.
...
2. **Revenue per session for NNV calibration**
   ...
   - What's unclear: What is the actual avg_revenue_per_session for Bit2Me users?
...
- NNV formula: MEDIUM - formula structure is standard, but EUR constants need Bit2Me calibration
- Holdout design: HIGH - well-established methodology, CleverTap supports natively
- Executive framing: MEDIUM - gap numbers are from Bit2Me actuals, but lift estimates are industry-based projections
