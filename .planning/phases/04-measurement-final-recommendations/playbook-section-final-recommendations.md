...
Pablo, the notification system addresses three revenue gaps that together represent the largest near-term growth opportunity for Bit2Me. Each gap is quantified with current Bit2Me data, and the trigger system provides a concrete, measurable intervention for each.
...
**Current state:** 72,400 users hold EUR 19.5M in assets but have not traded in 90+ days. These are not churned users -- they have money in the platform. They chose Bit2Me, deposited, bought crypto, and stopped. The assets are still there.
...
| Days 45-60 | A-05 Portfolio Value Milestone | Portfolio value aggregation across all assets per user | Alvaro (3 person-days): `user_portfolio_value` BigQuery view + Hightouch sync | `bit2me_lifecycle.user_portfolio_value` view created and validated | 5 person-days |
...
**V2 measurement milestone:** By Day 90, per-family holdout results are available. First NNV calibration with real Bit2Me data replaces the EUR 2.50 industry estimate. Weekly review cadence established.
...
**What:** Diego Barreira is the single legal gate for ALL CRM messages at Bit2Me. Every notification that contains marketing or informational content requires Diego's review before it can be sent. 7 of 9 existing CleverTap journeys are currently behind Diego's approval queue.
...
**Impact on trigger system:** Without C8 suppression, ANY P2-P5 trigger risks sending automated notifications to whale accounts. A dormant whale receiving a generic "Come back to Bit2Me" email (D-02) instead of a personal call from their account manager is a trust violation that could result in asset withdrawal. These users hold disproportionate AUC.
...
- [ ] **3. Alvaro: Create holdout_global_10pct segment in BigQuery** (1 hour)
  ...
  ```
  Save as `bit2me_lifecycle.holdout_global_10pct` materialized view.
...
- [ ] **7. Katy: Build A-01/A-02/A-03 campaigns in CleverTap** (3 hours)
  ...
  - Use deep link `bit2me://brokerage/trade?asset={symbol}` as the CTA
...
- [ ] **8. Diego: Pre-review Family D template copy** (2 hours)
  ...
  - **D-01 (At-Risk):** "Tu portafolio ha estado inactivo {days} dias. Revisa el rendimiento de tus activos." Deep link: `bit2me://portfolio/overview`
  - **D-02 (Dormant with Balance):** "Tienes EUR {balance} en tu cartera Bit2Me. Tu portafolio se ha movido {pct_change}% este mes." Deep link: `bit2me://portfolio/overview`
...
|--------------------------|-----------|---------|
| 15.1 Executive Summary | Section 14 (NNV formula), Section 11 (33 triggers), CLAUDE.md (Bit2Me numbers) | Business case with quantified gaps |
